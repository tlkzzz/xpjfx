...
<body>
	<ul class="nav nav-tabs">
		<li <c:if test="${empty type ||type eq '1'}">class="active"</c:if>><a href="${ctx}/ck/cDdinfo/xslist">品项统计</a></li>
		<li <c:if test="${not empty type&&type eq '2'}">class="active"</c:if>><a href="${ctx}/ck/cDdinfo/xslist?type=2">大类别统计</a></li>
			<li <c:if test="${not empty type&&type eq '3'}" >class="active" </c:if>><a href="${ctx}/ck/cDdinfo/xslist?type=3">小类别统计</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="cDdinfo" action="${ctx}/ck/cDdinfo/xslist" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="control-group">
				...
				<li class="controls">
					<sys:treeselect id="gclass" name="goods.gclass.id" value="${cDdinfo.goods.gclass.parent.id}" labelName="goods.gclass.parent.name" labelValue="${cDdinfo.goods.gclass.parent.name}"
									title="父级编号" url="/ck/cGclass/treeData" extId="${cDdinfo.goods.gclass.id}" cssClass="" allowClear="true"/>
				</li>
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			...
			<c:forEach items="${list}" var="cDdinfo" varStatus="status">
				<tr>
					...
					<td>
							${cDdinfo.goods.gclass.parent.name}
					</td>
					<td>
							${cDdinfo.goods.name},${cDdinfo.goods.spec.name}
					</td>
					<td>
